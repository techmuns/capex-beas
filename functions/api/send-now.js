...
// address. Rate-limited to 3 per email per hour. Sends the last 30 days of capex
...
export async function onRequestPost({ request, env }) {
  ...
  const cutoffISO = new Date(Date.now() - 30 * 86400_000).toISOString(); // one-off window: last 30 days
  ...
  if (!items.length && !sendEmpty) {
    return json({ ok: true, sent: false, note: 'No capex changes in the last 30 days to send right now.' });
  }
